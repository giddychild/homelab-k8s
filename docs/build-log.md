...
- **ArgoCD Notifications → Discord:** GitOps sync/health failures (on Error/Failed sync, Degraded health, or sync Unknown) → Discord, complementing the Prometheus path. Config in the argo-cd Helm values (`notifications:` block, `kubernetes/bootstrap/argocd/values.yaml`): webhook notifier `service.webhook.discord` resolves `$discord-url` from `argocd-notifications-secret` (ESO from Vault, `secret.create: false`), custom Discord-JSON template, `on-problem` trigger, default subscription. **GOTCHA:** trigger conditions must **nil-guard** `operationState` (`app.status.operationState != nil and ...`) or they error (`cannot fetch phase from <nil>`) for apps with no sync op; broken/misconfigured apps show `sync.status == 'Unknown'`, so cover that too. Apply via `helm upgrade argocd ... --version 9.5.15` (ArgoCD is Helm-bootstrapped, not self-managed).
- **Resource digest → Discord (every 15m):** CronJob `resource-digest` (`gitops/workloads/resource-digest/`, App `gitops/apps/resource-digest.yaml`, monitoring ns) queries in-cluster Prometheus for cluster CPU/Mem/Disk % and the 15m delta (PromQL `offset` — no stored state), posting a one-line summary to Discord **only when a metric moved ≥ `CHANGE_THRESHOLD`% (default 1)** so it's silent when idle. Reuses the `alertmanager-discord` webhook secret; pure Python stdlib. **GOTCHAs:** a per-1%-continuous trigger would spam and hit Discord's ~30/min rate limit (CPU is noisy) — hence periodic + only-on-change; and Discord **403s the default `Python-urllib` User-Agent**, so a custom UA is set. "Disk" = aggregate of the Talos node filesystems (xfs/ext4), not the ESXi datastore (no ESXi exporter).
...
| Resource digest → Discord (`resource-digest` CronJob, every 15m) | cluster CPU/Mem/Disk % + 15m delta; posts **only on ≥1% change** (tunable via `CHANGE_THRESHOLD`) |
