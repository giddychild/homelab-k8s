...
- [~] **Step 5 — Hardening**: audit logging, RBAC least-privilege, trust `homelab-ca`.
  ...
  - [~] **Audit logging** — *authored as code, pending apply.* Explicit kube-apiserver audit policy `talos/patches/controlplane.yaml` (security-focused levels: RBAC changes at `RequestResponse`, Secrets/ConfigMaps at `Metadata` only so values never hit the log, system noise dropped), wired into `scripts/talos-gen.sh` as `--config-patch-control-plane`. Apply/verify procedure: `docs/runbooks/audit-logging.md` (rolling `talosctl apply-config` per CP node — must run from `mgmt-jump`).
